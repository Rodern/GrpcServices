	...
	public class HttpHelper
	{
		...
		public static async Task<RequestResponse> PostAsync(string url, string content, HttpClient client, string token = "")
		{
			...
			catch (Exception ex)
			{
...
#if DEBUG
				return new RequestResponse(false, "Unable to connect to server:" + ex.Message);
#else
				...
				{
					return new RequestResponse(false, "It seems you have no data bundle active.");
				}
				...
				{
					return new RequestResponse(false, "You have no Internet connection");
				}
				else
				{
					return new RequestResponse(false, "Unable to connect to server");
				}
#endif
                /*if (ex.Message.Contains("System.Net.WebException: 'SSL handshake aborted:") || ex.Message.Contains("System.Net.Http.HttpRequestException: 'The SSL connection could not be established"))
				{
